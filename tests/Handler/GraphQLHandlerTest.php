...
class GraphQLHandlerTest extends TestCase
{
    ...
    public function testRenderPlayground(): void
    {
        ...
        $expected = '<title>GraphiQL 5 with React 19 and GraphiQL Explorer</title>';
  ...
  authors {
    ...
    {
        return file_get_contents(dirname(__DIR__) . '/graphql/test.query.graphql');
    }
    ...
    public function testGetAuthor(): void
    {
        ...
        $resultFile = dirname(__DIR__) . '/graphql/getAuthor.' . $id . '.result.json';
    ...
    public function testGetAuthors(): void
    {
        ...
        $resultFile = dirname(__DIR__) . '/graphql/getAuthors.l.' . $where['l'] . '.result.json';
        if (!file_exists($resultFile)) {
            file_put_contents($resultFile, Format::json($result));
        }
        $expected = json_decode(file_get_contents($resultFile), true);
    ...
    public static function getNodeProvider()
    {
        ...
        foreach ($idlist as $id) {
            $resultFile = dirname(__DIR__) . '/graphql/node' . str_replace('/', '.', $id) . '.result.json';
            if (file_exists($resultFile)) {
    ...
    public function testGetNode($id, $expected): void
    {
        ...
        $resultFile = dirname(__DIR__) . '/graphql/getNode' . str_replace('/', '.', $id) . '.result.json';
        if (!file_exists($resultFile)) {
            file_put_contents($resultFile, Format::json($result));
        }
        $expected = json_decode(file_get_contents($resultFile), true);
    ...
    public static function getQueryFields()
    {
        ...
        foreach ($queryType->getFieldNames() as $name) {
            $queryFile = dirname(__DIR__) . '/graphql/' . $name . '.query.json';
            $resultFile = dirname(__DIR__) . '/graphql/' . $name . '.result.json';
